...
############################################################
# 五、绘图基础 (traditional S/R graphics)
############################################################
...
# 散点图 加上拟合图
data = data.frame(x=rep(seq(1,5),10))
data$y=data$x+data$x^2+data$x^3+rnorm(50,0,8)
ggplot(data=data,aes(x=x,y=y))+ 
  geom_point() +
  geom_smooth(method="loess",formula =  "y~x+x^2+x^3")
