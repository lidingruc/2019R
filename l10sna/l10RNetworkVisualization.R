...
##   https://github.com/kateto                            ##
##========================================================##
## 网络数据的进一步学习请参考：http://sna.stanford.edu/rlabs.php
...
# ================ 1. R colors and fonts ================ 
# 网络数据的可视化和ggplot对于普通数据的可视化是一样的
# 同样是将数据特征映射到图形的位置、大小、原色、形状等几何属性上
#这一部分略过，想深入研究的同学可以自己阅读和研究
#跳到 211行左右 reading in data
#  ------->> Colors in R plots --------
# --COLORS--
...
# Plotting with igraph: node options (starting with 'vertex.') and 
#edge options (starting with 'edge.'). 
#A list of options is included in your handout.
...
plot(net, vertex.shape="none", vertex.label=V(net)$media, 
     vertex.label.font=1.5, vertex.label.color="tomato",
     vertex.label.cex=1.2, edge.color="gray40")
...
l <- layout_with_fr(net) #The Fruchterman-Reingold layout algorithm
...
# Modularity(模块度), 这个概念是2003年一个叫Newman的人提出的
#模块度越大则表明社区划分效果越好。值范围在[-0.5,1），论文表示当Q值在0.3~0.7之间时，说明聚类的效果很好。
...
comfg1 <- fastgreedy.community(as.undirected(net))
#随机游走
comfg2 <- walktrap.community(net, weights = E(net)$weight,steps = 2) # 没有群
#自旋玻璃
comfg3 <- spinglass.community(net) # 2个群
#特征值
comfg4<-leading.eigenvector.community(as.undirected(net),start=membership(comfg1)) # 一个群
#标签传播
comfg5<-label.propagation.community(net) # 一个群
#多层次聚类
comfg6<-multilevel.community(as.undirected(net),weights = E(net2)$weight) # 
...
# change comfg number here for different community detection method
plot(comfg1, net,
    ...
    # vertex.shape = E(net)$shape, 
     vertex.label.cex=labelsize,       
     vertex.label.family="STSong",
     vertex.label.color="black",
     vertex.label.dist=labeldist, 
     edge.arrow.size=arrowsize, 
     edge.color="gray60",
     edge.lty="solid",
     edge.curved=TRUE)

title("新闻网络图", family="STSong", cex.main=2)

#dev.print(png,'filenname.png'),width = 3283,
#          height = 2208,res=300)
# to save the graph

############
# 不同的聚类方式，进一步的分析
comfg <- cluster_edge_betweenness(net)

V(net)$community <- comfg$membership
colrs <- adjustcolor( c("gray50", "tomato", "gold", "yellowgreen","maroon"), alpha=.6)
